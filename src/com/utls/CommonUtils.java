...
public final class CommonUtils
{
	 ...
	 * @author xiaobai 2017年9月23日
	 */
	public static final class DateAndeTime
	{
		...
		/**
		 * 【方法功能】：计算日期加天数的结果(比如求 2017-01-02 +/- 3天=？的结果)
		 *  这个方法在
		 * @param startDate
		 *            开始的日期
		 * @param offsetDays
		 ...
		 * @return
		 */
		public static final Date getDateBy(Date startDate, int offsetDays)
		{
			Date dateAdd = CommonUtils.DateAndeTime.DateUtil.dateAdd(CommonUtils.DateAndeTime.DateUtil.INTERVAL_DAY, startDate, offsetDays);
			System.out.println(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(dateAdd));
			try
			{
				parse = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(dateAdd));
		...
		public static final class DateUtil
		{
			 ...
			 *            INTERVAL_YEAR, INTERVAL_HOUR,INTERVAL_MINUTE]
			 * @param date
			 * @param n
			 *            可以为负数
			 * @return
			 ...
			 * 
			 * @param format
			 *            时间格式由常量决定 8: YYMMDDHH 8位 10: YYMMDDHHmm 10位 12:
			 *            YYMMDDHHmmss 12位 14: YYYYMMDDHHmmss 14位 15:
